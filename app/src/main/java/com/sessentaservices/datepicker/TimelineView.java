...
import com.datepicker.adapter.TimelineAdapter;
...
public class TimelineView extends RecyclerView {
    ...
    void init() {
        ...
        setLayoutManager(new LinearLayoutManager(getContext(), LinearLayoutManager.HORIZONTAL,
                false));
        adapter = new TimelineAdapter(this, 0);
        setAdapter(adapter);
    }
    ...
    public void setActiveDate(Calendar activeDate) {
        try {
            Date initialDate = new SimpleDateFormat(CommonUtilities.DayFormatEN)
     ...
     * @param activeDate active Date
     */
    public void setActiveDateWithScroll(Calendar activeDate) {
        try {
            Date initialDate = new SimpleDateFormat(CommonUtilities.DayFormatEN)
                    .parse(year + "-" + (month + 1) + "-" + this.date);
            long diff = activeDate.getTime().getTime() - initialDate.getTime();
            ...
            invalidate();
            if (position != 0) {
                this.scrollToPosition(position - 1);
            }
        } catch (ParseException e) {
            e.printStackTrace();
        }

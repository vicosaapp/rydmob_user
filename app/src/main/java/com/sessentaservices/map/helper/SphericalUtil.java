...
public class SphericalUtil extends MathUtil {
    ...
    /*public static double computeAngle(LatLng from, LatLng to) {
        double latitudinalDifference = from.latitude - to.latitude;
        double longitudinalDifference = from.longitude - to.longitude;
        double azimuth = (Math.PI * .5d) - Math.atan(latitudinalDifference / longitudinalDifference);

        if (longitudinalDifference < 0) azimuth = azimuth + Math.PI;
        else if (latitudinalDifference < 0) azimuth = Math.PI;

        return toDegrees(azimuth);
    }*/

    public static double computeXYAngleBetween(LatLng from, LatLng to) {
        final double deltaY = (to.longitude - from.longitude);
        final double deltaX = (to.latitude - from.latitude);
        final double result = Math.toDegrees(Math.atan2(deltaY, deltaX));
        return (result < 0) ? (360d + result) : result;
    }

    public static Direction computeDirection(LatLng from, LatLng to) {
        double angle = computeXYAngleBetween(from, to);

        if ((angle > 0 && angle < 80) || (angle > 290 && angle <= 360)) {
            // Upward
